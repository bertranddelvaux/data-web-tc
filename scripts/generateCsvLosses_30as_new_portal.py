...
url_subfolder_list_15as = [
    'https://www.kacportal.com/portal/kacs3/arc/arc_proj22/historical_data/jtwc_high_resolution_hazard/',
    # <= 2021, historical
...
files_list_30as_updated = [filename for filename in files_list_30as if
                           any(pattern in filename for pattern in files_pattern)]

# Filter out URLs that contain any of the files in files_to_exclude
updated_files_list = [file for file in files_list_30as_updated if
                      not any(exclude in file for exclude in set(files_to_exclude))]
...
for url_file in updated_files_list[:N]:
    filename = os.path.basename(mappingKAC.get_backward(urlparse(url_file).path)) # TODO: changed os.path.basename(urlparse(url_file).path)  # get the filename from the URL
...
for file_name in os.listdir(impact_dir):
    if file_name.endswith('.csv') and any(
            f'adm{i}' in file_name for i in range(3)) and file_name not in aggregated_files_list:
