...
file_list = listFilesUrl(mappingKAC.get_forward(url), username, password, ext='.zip') # TODO: changed listFilesUrl(url, username, password, ext='.zip')
os.chdir('mpres_data')
            ...
            for storm_id in gdf.ATCFID.unique():
                ...
                for tech in gdf.TECH.unique():
                    ...
                    if tech == 'FCST' and last_lon and last_lat:  # if the technology is 'FCST' and last_lon and last_lat are not None
                        lons = np.insert(lons, 0,
                                         last_lon)  # add the last longitude to the beginning of the list of longitudes
                        lats = np.insert(lats, 0,
                                         last_lat)  # add the last latitude to the beginning of the list of latitudes

                    if len(lons) > 1 and len(lats) > 1:  # if there is more than one longitude and latitude
                        lineString = geom.LineString(
                            [(lon, lat) for lon, lat in zip(lons, lats)])  # create a LineString object
