...
url = 'https://kac.kacportal.com/portal/data/arc/tc_realtime/tc_active_storm_list.csv' # TODO: changed 'https://www.kacportal.com/portal/kacs3/arc/tc_realtime/arc_tc_data.csv'
...
url_realtime = mappingKAC.get_forward('https://www.kacportal.com/portal/kacs3/arc/tc_realtime/') # TODO: changed 'https://www.kacportal.com/portal/kacs3/arc/tc_realtime/'
...
files_tc_realtime_url = [os.path.splitext(os.path.split(mappingKAC.get_backward(url))[1])[0] for url in data[data.columns[-1]].values if
                         isinstance(data[data.columns[-1]].values[0], str)] # TODO: changed os.path.splitext(os.path.split(url)[1])[0]
...
if data.iloc[0][0] == 'NONE':
    ...
    for url_file in [url for url in data[data.columns[-1]].unique() if 'JTWC' in url]: #for url_file in data[data.columns[-1]].unique():
        ...
        os.chdir(dir_root)
        print(f'dir : {os.getcwd()}')
        filename = os.path.basename(mappingKAC.get_backward(urlparse(url_file).path)) # TODO changed os.path.basename(urlparse(url_file).path)

        downloaded = fetchUrl(url_file, username, password, filename=filename) # TODO: changed fetchUrl(url_file, username, password)
