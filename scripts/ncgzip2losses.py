...
def calculateLosses(storm_file, exp_file, adm_file, mapping_file, split, geojson, prefix='swath', csv_file='losses.csv', gadm_file=None):
    ...
    with open(csv_file, 'w') as f:
        ...
        for i, row in exp_df.iterrows():
            ...
            lat = row['lat']
            lon = row['lon']
            x = np.argmin(np.abs(lons - lon)) #x = int((lon - xmin)/dx)
            ...
            if x >= nlons:
                continue
            y = np.argmin(np.abs(lats - lat)) #y = int((lat - ymin)/dy)
            if y < 0:
                continue
            if y >= nlats:
                continue
...
if __name__ == '__main__':
    ...
    calculateLosses(storm_file=args.storm_file, exp_file=args.exp_file, adm_file=args.adm_file, mapping_file=args.mapping_file, split=args.split, geojson=args.geojson, gadm_file=args.gadm_file)
